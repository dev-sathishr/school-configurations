...
@Component({
  selector: 'app-employee-relation-list',
  standalone: true,
  ...
  template: `
    <div>
      ...
      <div class="mb-4 flex items-center justify-between">
        <p class="text-muted-foreground text-xs">Family members linked to this employee</p>
          ...
          @for (m of members; track m.id) {
            <div class="border-muted/30 rounded-lg border p-4 transition-colors"
              ...
              <div class="grid grid-cols-2 gap-x-6 gap-y-1.5 md:grid-cols-4">
                ...
                @if (m.qualification) {
                  <div>
                    <p class="text-muted-foreground text-[10px] uppercase tracking-wide">Qualification</p>
                    <p class="text-foreground text-xs font-medium">{{ m.qualification }}</p>
                  </div>
                }
                @if (m.annual_income) {
                  <div>
...
export class EmployeeRelationListComponent implements OnChanges {
  @Input() employeeId = '';
  ...
  load(): void {
    ...
    this.loading = true;
    this.cs.getService({ url: API.employeeFamilyInfo.base(this.employeeId) }).subscribe({
      next: (res: any) => {
