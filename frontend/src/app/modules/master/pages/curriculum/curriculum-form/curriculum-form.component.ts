...
export class CurriculumFormComponent {
  ...
  get title(): string {
    if (this.mode === 'create') return 'New Curriculum';
    if (this.mode === 'edit') return 'Edit Curriculum';
  ...
  openCreate(): void {
    ...
    this.saving = false;
    this.loading = false;
    this.form.reset({ name: '', notes: '', is_active: true });
    this.form.enable();
    this.showModal = true;
  }
  ...
  submit(): void {
    ...
    const request$ = this.mode === 'edit'
      ? this.cs.putService({ url: API.curriculums.detail(this.editId), payload })
      : this.cs.postService({ url: API.curriculums.base, payload });
  ...
  private loadRecord(id: string): void {
    this.cs.getService({ url: API.curriculums.detail(id) }).subscribe({
      next: (res: any) => {
