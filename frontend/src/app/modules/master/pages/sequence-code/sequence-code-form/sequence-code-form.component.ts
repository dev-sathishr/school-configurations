...
@Component({
  selector: 'app-sequence-code-form',
  templateUrl: './sequence-code-form.component.html',
  standalone: true,
  imports: [ReactiveFormsModule, ModalComponent, FormFieldComponent, ButtonComponent, LoaderComponent],
})
export class SequenceCodeFormComponent {
  ...
  get title(): string {
    if (this.mode === 'create') return 'New Sequence Code';
    if (this.mode === 'edit') return 'Edit Sequence Code';
    return 'View Sequence Code';
  }
  ...
  openCreate(): void {
    ...
    this.saving = false;
    this.loading = false;
    this.form.reset({ code: '', name: '', is_active: true });
    this.form.enable();
    this.showModal = true;
  }
  ...
  submit(): void {
    ...
    const request$ = this.mode === 'edit'
      ? this.cs.putService({ url: API.sequenceCodes.detail(this.editId), payload })
      : this.cs.postService({ url: API.sequenceCodes.base, payload });
  ...
  private loadRecord(id: string): void {
    this.cs.getService({ url: API.sequenceCodes.detail(id) }).subscribe({
      next: (res: any) => {
