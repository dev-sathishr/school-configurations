...
export class FeeCategoryFormComponent {
  ...
  get title(): string {
    if (this.mode === 'create') return 'New Fee Category';
    if (this.mode === 'edit')   return 'Edit Fee Category';
    return 'View Fee Category';
  }
  ...
  openCreate(): void {
    ...
    this.saving = false;
    this.loading = false;
    this.form.reset({ code: '', name: '', description: '', is_active: true });
    this.form.enable();
    this.showModal = true;
  }
  ...
  submit(): void {
    ...
    const request$ = this.mode === 'edit'
      ? this.cs.putService({ url: API.feeCategories.detail(this.editId), payload })
      : this.cs.postService({ url: API.feeCategories.base, payload });
  ...
  private loadRecord(id: string): void {
    this.cs.getService({ url: API.feeCategories.detail(id) }).subscribe({
      next: (res: any) => {
