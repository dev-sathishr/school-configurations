...
async function uploadFile(req, resp) {
  try {
    ...
    return res.error(resp, err.message || 'Upload failed');
  }
}
...
async function getEntityFiles(req, resp) {
  try {
    ...
    console.error('Get entity files error:', err);
    return res.error(resp);
  }
}
...
module.exports = { upload, uploadFile, getFile, getEntityFiles, deleteFile };
