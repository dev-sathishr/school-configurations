...
async function findAll(query, viewOwnUserId) {
  return paginate({
    ...
    sortableColumns: ['m.name', 'm.code', 'm.display_order', 'm.is_active', 'm.created_at', 'module_count'],
...
async function getMenusWithModules() {
  const result = await db.query(`
    SELECT m.id AS menu_id, m.name AS menu_name, m.code AS menu_code, m.icon AS menu_icon, m.display_order AS menu_order,
      mm.module_id, mod.name AS module_name, mod.code AS module_code, mod.icon AS module_icon, mod.route_path AS module_route_path, mm.display_order AS module_order
    FROM settings.menus m
